function x = CSL1NlCg_CompositeL1_TGV(x0,param,param2,param3,param4,param5,param6) % CompositeL1, TGV
%
% res = CSL1NlCg(param)
%
...
param3.l1Smooth = 1e-15;	% TGV
param4.l1Smooth = 1e-15;	% TGV
param5.l1Smooth = 1e-15;	% TGV
param6.l1Smooth = 1e-15;	% TGV
alpha = 0.01;
beta = 0.6;
t0 = 1 ;
k = 0;
...
% compute g0  = grad(f(x))
g0 = grad(x,param,param2,param3,param4,param5,param6); % CompositeL1, TGV
dx = -g0;
...
while(1)
    ...
    %conjugate gradient calculation
    g1 = grad(x,param,param2,param3,param4,param5,param6); % CompositeL1, TGV
    bk = g1(:)'*g1(:)/(g0(:)'*g0(:)+eps);
    g0 = g1;
    dx =  - g1 + bk* dx;
    k = k + 1;
...
function res = objective(x,dx,t,param,param2,param3,param4,param5,param6) %********************************** % CompositeL1, TGV
...
if param4.lambda
    w = param4.W*(x+t*dx);
    L1Obj4 = sum((conj(w(:)).*w(:)+param4.l1Smooth).^(1/2));   
else
    L1Obj4=0;
end
...
if param5.lambda
    w = param5.W*(x+t*dx);
    L1Obj5 = sum((conj(w(:)).*w(:)+param5.l1Smooth).^(1/2));   
else
    L1Obj5=0;
end
...
if param6.lambda
    w = param6.W*(x+t*dx);
    L1Obj6 = sum((conj(w(:)).*w(:)+param6.l1Smooth).^(1/2));   
else
    L1Obj6=0;
end
% TGV block end
...
res=L2Obj+param.lambda*L1Obj...
    +param2.lambda*L1Obj2... % CompositeL1
    +param3.lambda*L1Obj3... % TGV
    +param4.lambda*L1Obj4... 
    +param5.lambda*L1Obj5... 
    +param6.lambda*L1Obj6;    
...
function g = grad(x,param,param2,param3,param4,param5,param6)%*********************************************** % CompositeL1, TGV
...
if param4.lambda
    w = param4.W*x;
    L1Grad4 = param4.W'*(w.*(w.*conj(w)+param4.l1Smooth).^(-0.5));    
else
    L1Grad4=0;
end
...
if param5.lambda
    w = param5.W*x;
    L1Grad5 = param5.W'*(w.*(w.*conj(w)+param5.l1Smooth).^(-0.5));    
else
    L1Grad5=0;
end
...
if param6.lambda
    w = param6.W*x;
    L1Grad6 = param6.W'*(w.*(w.*conj(w)+param6.l1Smooth).^(-0.5));    
else
    L1Grad6=0;
end
% TGV block end
...
% composite gradient
g=L2Grad+param.lambda*L1Grad+param2.lambda*L1Grad2; % CompositeL1

g = g+param3.lambda*L1Grad3... % TGV
     +param4.lambda*L1Grad4...
     +param5.lambda*L1Grad5...
     +param6.lambda*L1Grad6;
